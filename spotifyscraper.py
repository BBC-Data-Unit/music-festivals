...
#The example below is not a real token, so you need to create your own
token = 'BQA5nADDacSi7JmME830Cx3NCl5I7lSGClpBSUwkG3m6oNalpO2oVIZ1ogAklMWzj_Kkk3oJBCn8fasNblvPDw'
...
#create an empty dict to store our data
record = {}
...
for artist in artists:
    ...
    #load the JSON from that
    data = json.loads(response.read())
    #Access different parts of the JSON and print them
    print data['name']
    print data['genres']
    #create an empty string - then add each genre to turn a list into a single comma separated string
    genres = ''
    for genre in data['genres']:
        genres = genres+','+genre
    #once finished, the first character of that genres list is a comma, so we grab everything after that first character and store
    record['genres'] = genres[1:]
    #these lines access parts of the JSON and store in the record variable with different keys
    record['name'] = data['name']
    record['popularity'] = data['popularity']
    record['type'] = data['type']
    record['followers'] = data['followers']['total']
    #these lines store data created at the start of the loop
    record['fullurl'] = fullurl
    record['artistid'] = artist
    #Now grab the top tracks - this could have been in a separate function but, hey.
    ...
    #Now onto related artists - again this should probably be another function
    ...
    #finally, record the data in the sqlite database which can be queried
